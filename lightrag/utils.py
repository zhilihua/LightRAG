...
def limit_async_func_call(max_size: int):
    """Add restriction of maximum concurrent async calls using asyncio.Semaphore(使用 asyncio.Semaphore 添加最大并发异步调用的限制)"""
...
def lazy_external_import(module_name: str, class_name: str) -> Callable[..., Any]:
    ...
    caller_frame = inspect.currentframe().f_back  # 获取当前的栈帧的上一帧
...
def always_get_an_event_loop() -> asyncio.AbstractEventLoop:
    """
    Ensure that there is always an event loop available(确保始终有事件循环可用).

    This function tries to get the current event loop. If the current event loop is closed or does not exist,
    it creates a new event loop and sets it as the current event loop.
    此函数尝试获取当前事件循环。如果当前事件循环已关闭或不存在，则创建一个新的事件循环并将其设置为当前事件循环

    Returns:
        asyncio.AbstractEventLoop: The current or newly created event loop.
    """
    try:
        # Try to get the current event loop(尝试获取当前事件循环)
