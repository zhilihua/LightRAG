...
from lightrag.kg.shared_storage import (
    get_update_flag,
    get_storage_lock, set_all_update_flags
)
...
class NetworkXStorage(BaseGraphStorage):
    ...
    async def get_knowledge_graph(
        ...
        检索标签包含指定“node_label”的节点的连通子图。
        Maximum number of nodes is constrained by the environment variable `MAX_GRAPH_NODES` (default: 1000).
        最大节点数受环境变量“MAX_GRAPH_NODES”限制（默认值：1000）。
        ...
        Args:
            ...
            node_label：起始节点的标签
            max_depth：子图的最大深度
            min_degree：要包含的节点的最小度。默认为 0
            inclusive：如果为真，则进行包含搜索
