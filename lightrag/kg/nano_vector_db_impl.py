...
class NanoVectorDBStorage(BaseVectorStorage):
    ...
    async def initialize(self):
        ...
        # Get the update flag for cross-process update notification(获取跨进程更新通知的更新标志)
        self.storage_updated = await get_update_flag(self.namespace)
        # Get the storage lock for use in other methods(获取存储锁以供其他方法使用)
    ...
    async def upsert(self, data: dict[str, dict[str, Any]]) -> None:
        ...
        # Execute embedding outside of lock to avoid long lock times(在锁外执行嵌入，以避免较长的锁定时间)
